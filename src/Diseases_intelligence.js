...
(function () {
    ...
    const WHO_CASE_DEFINITIONS = {
        ...
        rabies: {
            suspected: 'Any person with acute encephalitis AND recent (within 1 year) bite/scratch from an animal in a rabies-endemic country — OR unexplained progressive encephalitis with hydrophobia.',
            ...
            poe_action: 'If patient symptomatic with hydrophobia — immediate isolation + palliative care referral. Provide PEP history assessment.',
        },
    ...
    const SYNDROME_RULES = [
        ...
        },
        {
            ...
            any: ['hydrophobia'],
            reasoning: 'Hydrophobia — pathognomonic for rabies. EMERGENCY protocol.',
        },
        {
    ...
    const ENGINE_TO_WHO_SYNDROME = {
        ...
        jaundice_hemorrhagic: 'JAUNDICE',
        hemorrhagic_fever: 'VHF',
        hemorrhagic_fever_renal: 'VHF',
        hemorrhagic_possible: 'VHF',
        tick_borne_hemorrhagic: 'VHF',
        ...
        acute_bloody_diarrhea: 'BLOODY_DIARRHEA',
        jaundice_syndrome: 'JAUNDICE',
        jaundice_renal: 'JAUNDICE',
        jaundice: 'JAUNDICE',
        acute_hepatitis: 'JAUNDICE',
        acute_febrile: 'OTHER',
        febrile: 'OTHER',
        febrile_arthralgia: 'OTHER',
        arboviral: 'OTHER',
        vector_borne: 'OTHER',
        travel_fever: 'OTHER',
        zoonotic: 'OTHER',
        ...
        cutaneous_ulcer: 'OTHER',
        bioterrorism: 'OTHER',
        bioterrorism_sentinel: 'OTHER',
        tick_borne: 'OTHER',
        undulant_fever: 'OTHER',
        parasitic: 'OTHER',
        skin_lesion: 'OTHER',
        ...
        treponemal: 'OTHER',
        neonatal: 'OTHER',
        exposure_event: 'OTHER',
        gastrointestinal: 'AWD',
        cluster: 'OTHER',
        mortality: 'OTHER',
        perinatal: 'OTHER',
        child: 'OTHER',
    ...
    const IDSR_PHEIC_DISEASES = (D.diseases || [])
        .filter(d => d && d.idsr_category === 'pheic' && d.category !== 'event')
        .map(d => d.id);
    const IDSR_AHF_DISEASES = ['ebola_virus_disease', 'marburg_virus_disease', 'lassa_fever', 'cchf'];

    const IHR_ESCALATION_RULES = [
        {
            id: 'IDSR_PHEIC_TOP3',
            description: 'IDSR PHEIC disease in top 3 with score ≥ 20 AND exposure evidence — OR isolated score ≥ 60',
            check: (result) => (result.top_diagnoses || []).slice(0, 3).some(d => {
                if (!IDSR_PHEIC_DISEASES.includes(d.disease_id)) return false;
                if (d.final_score < 20) return false;
                if (d.final_score >= 60) return true;
                return (d.matched_exposures || []).length > 0;
            }),
            risk_level: 'CRITICAL',
            routing: 'NATIONAL',
            reasoning: 'IDSR / IHR 2005 PHEIC disease — immediate national notification.',
        },
        {
            id: 'IDSR_AHF_HIGH',
            description: 'Any AHF (Ebola/Marburg/Lassa/CCHF) in top 2 with score ≥ 35',
            check: (result) => (result.top_diagnoses || []).slice(0, 2).some(d =>
                IDSR_AHF_DISEASES.includes(d.disease_id) && d.final_score >= 35
            ),
            risk_level: 'CRITICAL',
            routing: 'NATIONAL',
            reasoning: 'High-confidence Acute Haemorrhagic Fever — IDSR Annex 1A immediate notification pathway.',
        },
        {
            id: 'IDSR_AHF_POSSIBLE',
            description: 'Any AHF in top 5 with score ≥ 20',
            check: (result) => (result.top_diagnoses || []).some(d =>
                IDSR_AHF_DISEASES.includes(d.disease_id) && d.final_score >= 20
            ),
            risk_level: 'HIGH',
            routing: 'PHEOC',
            reasoning: 'Possible AHF — IDSR precautionary isolation pending laboratory confirmation.',
        },
        {
            id: 'IDSR_SARI_HYPOXIA',
            description: 'SARI / influenza pathway with SpO2 < 90 %',
            check: (result, vitals) => (result.top_diagnoses || []).some(d =>
                d.disease_id === 'sari' || d.disease_id === 'influenza_new_subtype_zoonotic' ||
                d.disease_id === 'sars' || d.disease_id === 'covid_19'
            ) && vitals.oxygen_saturation !== undefined && vitals.oxygen_saturation < 90,
            risk_level: 'CRITICAL',
            routing: 'PHEOC',
            reasoning: 'SARI with severe hypoxia — IDSR PHEIC respiratory pathway.',
        },
        {
            id: 'IDSR_HEMORRHAGE_EMERGENCY',
            description: 'Hemorrhage protocol activated by VHF triage override',
            ...
            risk_level: 'CRITICAL',
            routing: 'NATIONAL',
            reasoning: 'AHF protocol activated — IDSR immediate notification.',
        },
        {
            id: 'IDSR_BIOTERRORISM',
            description: 'Smallpox or anthrax cluster pattern',
            check: (result, vitals, globalFlags) =>
        ...
        {
            id: 'IDSR_AFP_POLIO',
            description: 'Acute flaccid paralysis — polio exclusion mandatory',
            check: (result, vitals, globalFlags) => globalFlags.includes('AFP_SURVEILLANCE_ACTIVATED'),
            risk_level: 'HIGH',
            routing: 'PHEOC',
            reasoning: 'AFP detected — IDSR/IHR mandatory polio exclusion. Stool sample x2 required.',
        },
        {
            id: 'CRITICAL_VITALS',
            description: 'Life-threatening vitals regardless of diagnosis',
            check: (result, vitals) =>
        ...
        {
            id: 'IDSR_CLUSTER_EVENT',
            description: 'Cluster of deaths or unusual event flagged',
            check: (result) => (result.top_diagnoses || []).some(d =>
                ['cluster_of_deaths', 'public_health_event_unknown'].includes(d.disease_id) && d.final_score >= 40
            ),
            risk_level: 'CRITICAL',
            routing: 'NATIONAL',
            reasoning: 'IDSR Table 1 cluster / unusual event — national notification required.',
        },
    ];
    ...
    D.scoreDiseases = function (presentSymptoms, absentSymptoms, selectedExposures, context) {
        ...
        // Diseases demoted by the IDSR Annex 1A AHF bleed-gate
        // (score_breakdown.vhf_bleeding_gate set) MUST NOT regain points via
        // the syndrome bonus — that would let a fever-only or bleeding-only
        // case climb back over the gate threshold.
        ...
        for (const r of _pd) {
            if (r.score_breakdown && r.score_breakdown.vhf_bleeding_gate) continue;
    ...
    D.validateClinicalData = function (presentSymptoms, vitals) {
        ...
        if (presentSymptoms.includes('hydrophobia')) {
            critical.push('RABIES_PATHOGNOMONIC');
            warnings.push('RABIES: Hydrophobia is pathognomonic — palliative care only. Notify IHR.');
            needsEmergency = true;
        }
    ...
    D.getWHOCaseDefinition = function (diseaseId) {
        const disease = (typeof D.getDiseaseById === 'function')
            ? D.getDiseaseById(diseaseId)
            : D.diseases.find(d => d.id === diseaseId);
        if (!disease) return null;
        if (disease.who_case_definition) return disease.who_case_definition;
        if (WHO_CASE_DEFINITIONS[diseaseId]) return WHO_CASE_DEFINITIONS[diseaseId];
        if (disease.case_definition) {
            return {
                suspected: disease.case_definition.suspected || null,
                probable:  disease.case_definition.probable  || null,
                confirmed: disease.case_definition.confirmed || null,
                ihr_category: disease.idsr_category || null,
                source: disease.idsr_source_ref
                    ? 'Uganda IDSR Annex 1A (' + disease.idsr_source_ref + ')'
                    : null,
            };
        }
        return null;
        ...
        if (!nonCaseVerdict.isNonCase) {
            ...
            scoreResult = D.scoreDiseases(
                ...
                exposureEngineCodes,
                {
                    ...
                    clinical_context: {
                        ...vitals,
                        ...((vitals && vitals.clinical_context) || {}),
    ...
    D.deriveIDSRSyndrome = D.deriveWHOSyndrome;
    D.getIDSRCaseDefinition = D.getWHOCaseDefinition;
    D.engine_to_idsr_syndrome = ENGINE_TO_WHO_SYNDROME;
    D.IDSR_PHEIC_DISEASES = IDSR_PHEIC_DISEASES;
    D.IDSR_AHF_DISEASES = IDSR_AHF_DISEASES;

    D.evaluateIDSRThresholds = function (diseaseId /*, context */) {
        const d = (typeof D.getDiseaseById === 'function')
            ? D.getDiseaseById(diseaseId)
            : (D.diseases || []).find(x => x.id === diseaseId);
        if (!d) return null;

        const alertText = (d.alert_threshold || '').trim();
        const epidemicTxt = (d.epidemic_threshold || '').trim();

        const SINGLE_CASE = /^1\s+(suspected|confirmed)\s+case/i;
        const PER_FACILITY = /\d+\s+confirmed\s+cases?\s*\/\s*(district hospital|DH|HF)\s*\/\s*(month|week)/i;
        const ATTACK_RATE = /attack rate of\s*\d+\s*cases?\s*per\s*100\s*0?00/i;
        const SEASONAL = /reported cases exceeding expected number/i;

        function classify(t) {
            if (!t) return { measurable: false, kind: 'unknown', text: '' };
            if (SINGLE_CASE.test(t))   return { measurable: true,  kind: 'single_case', text: t };
            if (PER_FACILITY.test(t))  return { measurable: false, kind: 'aggregate_per_facility', text: t };
            if (ATTACK_RATE.test(t))   return { measurable: false, kind: 'population_attack_rate', text: t };
            if (SEASONAL.test(t))      return { measurable: false, kind: 'seasonal_baseline', text: t };
            return { measurable: false, kind: 'narrative', text: t };
        }

        return { alert: classify(alertText), epidemic: classify(epidemicTxt) };
    };
        ...
        function build_override_section(overridesFired, overrideBoostContext) {
            const overrideDescriptions = {
                ...
                override_rabies_pathognomonic:
                    ...
                    'Trigger: Hydrophobia (terror/spasm on seeing/hearing water).\n' +
                    'Clinical basis: Hydrophobia is pathognomonic for rabies — no other disease produces this reflexive pharyngeal spasm. Once symptomatic, rabies has a case fatality rate approaching 100%. The engine applies the maximum single-symptom boost in the system: +50 to Rabies. The officer must assess bite/scratch history from a potentially rabid animal and arrange palliative care referral immediately.\n' +
    ...
    D.generateClinicalReport = function (enhancedResult, caseContext) {
        ...
        function countryName(code) {
            ...
            const known = {
                ...
                'RW': 'Uganda', 'ZM': 'Zambia', 'GH': 'Ghana', 'CI': 'Côte d\'Ivoire', 'CM': 'Cameroon',
        ...
        // ── SECTION 2: CLINICAL PRESENTATION ANALYSIS ───────────────────────
        {
            ...
            if (presentSymptoms.length === 0) {
                ...
                const NEURO_SYM = ['headache', 'severe_headache', 'stiff_neck', 'altered_consciousness', 'seizures', 'photophobia', 'paralysis_acute_flaccid', 'hydrophobia', 'encephalitis_signs'];
        ...
        if (!isNonCase && topD) {
            ...
            if (overridesFired.length > 0) {
                ...
                const overrideDescriptions = {
                    ...
                    'override_rabies_pathognomonic': 'Hydrophobia (inability to swallow liquids due to laryngeal spasm) is pathognomonic for rabies encephalitis. Once symptomatic, rabies is virtually 100% fatal. The role of POE is identification for contact tracing and family notification, not curative treatment.',
    ...
    // ══════════════════════════════════════════════════════════════════════
    // SECTION 9: SPECIFICITY CALIBRATION LAYER v3.1.0
    //
    // Wraps scoreDiseases() and getEnhancedScoreResult() to raise specificity
    // for IHR Tier-1 / Tier-2 high-consequence diseases without loosening
    // safety gates. Rationale and per-rule justification:
    //   C1. Absent-evidence deduction: any symptom with weight >= 10 confirmed
    //       ABSENT reduces that disease's score even when the symptom is not
    //       listed in the per-disease hallmarks table. Corrects for the
    //       asymmetric original rule (only hallmarks with weight >= 14).
    //   C2. Non-specific hemorrhage de-rate: if the only hemorrhage signal is
    //       bleeding_gums_or_nose (epistaxis/gingivitis) AND no VHF-specific
    //       exposure is present, VHF override boosts are scaled down. Full
    //       hemorrhage + exposure still fires the critical pathway.
    //   C3. Incubation / onset window plausibility: when days_since_onset is
    //       outside disease.incubation_days, score is capped at the minimal
    //       band (24).
    //   C4. Outbreak-bonus plausibility: the per-country outbreak bonus is
    //       capped at +3 when no hallmark AND no specific exposure matches.
    //   C5. OTHER-syndrome de-rate: the +8 syndrome bonus is reduced to +2
    //       when the derived WHO syndrome is OTHER (low specificity).
    //   C6. Tier-1 promotion gate: a Tier-1/Tier-2-IHR disease may only be
    //       top-ranked in the very_high band when it has supports >= 3
    //       (hallmarks matched + specific exposures matched + HI-specificity
    //       syndrome match + override-with-exposure) AND gate PASSED.
    //       With supports < 2 it is capped at the low band (39).
    //   C7. Non-case fallback: when no disease clears its promotion gate and
    //       no high-confidence override with exposure has fired, the verdict
    //       is "NO_CURRENT_MATCH" rather than the highest-severity disease.
    //       Emergency vital signs still trigger medical escalation.
    //   C8. Officer-declared suspicion: context.officer_declared_suspicion
    //       always raises the alert regardless of score.
    //
    // All calibration parameters are exposed on D.engine.formula.calibration
    // so an epidemiologist can tune without touching code.
    // ══════════════════════════════════════════════════════════════════════

    D.engine.formula.calibration = {
        version: '3.2.0',
        tier1_ihr_diseases: [
            'smallpox', 'sars', 'influenza_new_subtype_zoonotic', 'polio',
            'ebola_virus_disease', 'marburg_virus_disease', 'lassa_fever', 'cchf',
            'rift_valley_fever', 'meningococcal_meningitis', 'cholera',
            'yellow_fever', 'pneumonic_plague',
            'bubonic_plague', 'measles', 'anthrax_pulmonary',
        ],
        vhf_core: [
            'ebola_virus_disease', 'marburg_virus_disease', 'lassa_fever', 'cchf',
            'rift_valley_fever',
        ],
        // IHR Tier-1 always-notifiable (strict): these are vanishingly rare
        // and demand stronger evidence before raising CRITICAL.
        strict_tier1: ['smallpox', 'sars', 'influenza_new_subtype_zoonotic', 'polio'],
        min_supports_for_strict_tier1: 3,
        min_supports_for_top_rank: 2,
        min_supports_for_very_high_band: 3,
        demotion_cap_moderate: 54,
        demotion_cap_low: 39,
        demotion_cap_minimal: 24,
        absent_symptom_weight_floor: 10,
        absent_symptom_extra_penalty: -12,
        absent_symptom_penalty_max_per_disease: -30,
        nonspecific_hemorrhage: ['bleeding_gums_or_nose'],
        nonspecific_hemorrhage_demotion: -8,
        vhf_exposure_specific: [
            'contact_body_fluids', 'contact_dead_body', 'funeral_or_burial_exposure',
            'healthcare_exposure', 'travel_from_outbreak_area', 'close_contact_case',
            'laboratory_exposure', 'rodent_exposure', 'tick_bite',
            'flood_livestock_exposure',
        ],
        outbreak_bonus_unsupported_cap: 3,
        incubation_violation_cap: 24,
        syndrome_bonus_other_specificity: 2,
        non_case_fallback_top_score_threshold: 40,
        non_case_fallback_supports_threshold: 2,
        // Non-specific hallmarks that don't count as "support" on their own.
        // Expanded in calibration 3.2.0: generic GI / respiratory / muscular
        // symptoms are routinely present in dozens of common infections
        // (gastroenteritis, flu, malaria, common cold, food poisoning) and
        // must not — individually — promote a high-consequence VHF disease
        // to the top of the board. A VHF needs epidemiological plausibility,
        // not just "fever + vomiting + diarrhoea".
        generic_hallmarks_excluded_from_support: [
            'fever', 'headache', 'malaise', 'fatigue', 'nausea',
            'weakness', 'anorexia', 'low_grade_fever',
            'vomiting', 'diarrhea', 'abdominal_pain',
            'cough', 'sore_throat', 'coryza', 'chills', 'dizziness',
            'muscle_pain', 'joint_pain', 'back_pain', 'limb_pain',
            'rash_maculopapular',
        ],
        // Score ceiling applied to a Tier-1/Tier-2-IHR disease that has
        // supports < min_supports_for_top_rank. Set below the IHR TIER1
        // escalation threshold (20) so it cannot force a CRITICAL alert.
        demotion_cap_unsupported: 19,
        high_confidence_override_ids: [
            'override_vhf_red_flag',
            'override_acute_flaccid_paralysis',
            'override_meningitis_triad',
            'override_smallpox_pustular_centrifugal',
        ...
        nonspecific_override_ids: [
            'override_any_haemorrhage_fever_no_exposure',
            'override_vesiculopustular_rash',
        ],
        // Calibration 3.2.0 — bias controls for high-consequence VHFs.
        //
        // Motivation: field officers reported that Ebola / Marburg / CCHF /
        // Lassa were routinely surfacing as the top match for travellers who
        // only had non-specific febrile/GI illness — because the base engine
        // treated "travel_from_outbreak_area" OR a single hallmark like
        // "vomiting" as strong evidence.  This set of calibrators enforces
        // an epidemiological-plausibility gate: a VHF disease can only reach
        // the top of the board when at least one of the hard signals is
        // present (direct body-fluid/dead-body exposure, confirmed bleeding
        // beyond epistaxis, or outbreak context).
        //
        // Set vhf_plausibility_enabled = false to disable all VHF demotion
        // (emergency fallback; do not do this without an epidemiologist).
        vhf_plausibility_enabled: true,
        vhf_plausibility_cap: 39,                       // demote to "low" band
        vhf_true_hemorrhage_symptoms: [
            'bleeding', 'bloody_sputum', 'bloody_diarrhea', 'blood_in_vomit',
            'petechial_or_purpuric_rash', 'bruising_or_ecchymosis',
        ],
        // VHF-specific body-fluid/healthcare/outbreak exposures. Mirrors the
        // narrower "hard signal" set — wider vhf_exposure_specific is kept
        // for support counting but only these qualify for plausibility.
        vhf_hard_exposures: [
            'contact_body_fluids', 'contact_dead_body',
            'funeral_or_burial_exposure', 'healthcare_exposure',
            'close_contact_case', 'laboratory_exposure',
        ],
        // Override-boost dampening when the ONLY override that fired is a
        // non-specific one (e.g. fever + epistaxis with no exposure).  Scale
        // the override_boost on VHF core diseases by this factor so the
        // boost is informative but not dominant.
        nonspecific_override_vhf_boost_scale: 0.4,
    };

    const CAL = D.engine.formula.calibration;
    const BANDS = D.engine.normalization.confidence_bands;
    const TIER_ORDER = {
        tier_1_ihr_critical: 0, tier_2_ihr_annex2: 1,
        tier_2_ihr_equivalent: 2, tier_3_who_notifiable: 3, tier_4_syndromic: 4,
    };
    ...
    function bandFor(score) {
        for (const b of BANDS) if (score >= b.min_score) return b.band;
        return BANDS[BANDS.length - 1].band;
    }

    function incubationOk(disease, daysSinceOnset) {
        if (daysSinceOnset === undefined || daysSinceOnset === null) return null; // unknown = neutral
        const inc = disease.incubation_days;
        if (!inc || inc.min === undefined || inc.max === undefined) return null;
        const d = Number(daysSinceOnset);
        if (!Number.isFinite(d)) return null;
        return d >= inc.min && d <= inc.max;
    }

    // Computes supports for a result row. Returns structured counts.
    function computeSupports(r, disease, presentSymptoms, selectedExposures,
                             overridesFired, vhfSpecificExposures, incubationState,
                             adjustments) {
        const generic = new Set(CAL.generic_hallmarks_excluded_from_support);
        const specificHallmarks = (r.matched_hallmarks || []).filter(h => !generic.has(h));
        const hallmarks = specificHallmarks.length;
        const specificExposures = Array.isArray(r.matched_exposures)
            ? r.matched_exposures.length : 0;

        // Syndrome credit is suppressed when C2 (non-specific hemorrhage with
        // no VHF exposure) fired: the syndrome was "VHF" only because of a
        // non-specific bleeding signal and must not count as a support.
        const suppressSyndromeCredit =
            adjustments && adjustments.syndrome_support_suppressed ? true : false;
        const syndromeHi =
            r.syndrome_matched && r.syndrome_matched !== 'OTHER' && !suppressSyndromeCredit
                ? 1 : 0;

        const overrideWithExposure =
            overridesFired.some(id => CAL.high_confidence_override_ids.includes(id)) &&
            selectedExposures.some(e => vhfSpecificExposures.includes(e))
            ? 1 : 0;

        const incubationSupport = incubationState === true ? 1 : 0;

        return {
            hallmarks,
            specific_exposures: specificExposures,
            syndrome_hi_spec: syndromeHi,
            override_with_exposure: overrideWithExposure,
            incubation_ok: incubationSupport,
            total: hallmarks + specificExposures + syndromeHi +
                   overrideWithExposure + incubationSupport,
        };
    }

    // Apply calibration adjustments in place on a single result row.
    function calibrateOne(r, disease, presentSymptoms, absentSymptoms,
                          selectedExposures, overridesFired, daysSinceOnset) {
        const sw = disease.symptom_weights || {};
        const ew = disease.exposure_weights || {};
        const breakdown = r.score_breakdown || {};

        const adjustments = {
            absent_symptom_extra: 0,
            nonspecific_hemorrhage: 0,
            outbreak_plausibility: 0,
            syndrome_other: 0,
            incubation_cap_applied: false,
            incubation_cap_before: null,
            promotion_cap_applied: null,
            promotion_cap_before: null,
            supports: null,
            notes: [],
        };

        // C1 — Absent-evidence deduction for any symptom with weight >= floor
        for (const sym of absentSymptoms) {
            const w = sw[sym] || 0;
            if (w >= CAL.absent_symptom_weight_floor) {
                adjustments.absent_symptom_extra += CAL.absent_symptom_extra_penalty;
            }
        }
        if (adjustments.absent_symptom_extra < CAL.absent_symptom_penalty_max_per_disease) {
            adjustments.absent_symptom_extra = CAL.absent_symptom_penalty_max_per_disease;
        }

        // C2 — Non-specific hemorrhage de-rate
        const hemorrhageSymptomsPresent = presentSymptoms.filter(s =>
            ['bleeding', 'bleeding_gums_or_nose', 'bloody_sputum',
             'petechial_or_purpuric_rash', 'bruising_or_ecchymosis',
             'hematuria', 'blood_in_vomit'].includes(s));
        const hasExposureAnyVhfSpecific = selectedExposures.some(e =>
            CAL.vhf_exposure_specific.includes(e));
        const onlyNonspecificHem =
            hemorrhageSymptomsPresent.length > 0 &&
            hemorrhageSymptomsPresent.every(s => CAL.nonspecific_hemorrhage.includes(s));
        const isVhfCore = CAL.vhf_core.includes(disease.id);
        if (isVhfCore && onlyNonspecificHem && !hasExposureAnyVhfSpecific) {
            adjustments.nonspecific_hemorrhage = CAL.nonspecific_hemorrhage_demotion;
            adjustments.syndrome_support_suppressed = true;
            adjustments.notes.push('non_specific_hemorrhage_derate');
        }

        // C3 — Incubation plausibility
        const incState = incubationOk(disease, daysSinceOnset);

        // C4 — Outbreak bonus plausibility
        const outbreakApplied = breakdown.outbreak_bonus || 0;
        const hallmarksMatched = (r.matched_hallmarks || []).length;
        const specificExposureMatched = (r.matched_exposures || []).length;
        if (outbreakApplied > CAL.outbreak_bonus_unsupported_cap &&
            hallmarksMatched === 0 && specificExposureMatched === 0) {
            const overageRemoved = outbreakApplied - CAL.outbreak_bonus_unsupported_cap;
            adjustments.outbreak_plausibility = -overageRemoved;
            adjustments.notes.push('outbreak_bonus_unsupported_capped');
        }

        // C5 — OTHER-syndrome de-rate
        const syndromeBonusApplied = breakdown.syndrome_bonus || 0;
        if (r.syndrome_matched === 'OTHER' && syndromeBonusApplied > CAL.syndrome_bonus_other_specificity) {
            adjustments.syndrome_other = -(syndromeBonusApplied - CAL.syndrome_bonus_other_specificity);
            adjustments.notes.push('syndrome_other_low_specificity');
        }

        // C9 — Non-specific override boost dampening (VHF core only).
        // When only a non-specific override fired (fever + epistaxis-style
        // bleeding, or vesiculopustular rash WITHOUT face-first distribution),
        // scale down the override_boost that lifted this VHF so one common
        // sign can't single-handedly push a tier-2 VHF to the top.
        adjustments.override_boost_scale_delta = 0;
        const isVhfCoreForOverride = CAL.vhf_core.includes(disease.id);
        const overrideBoost = breakdown.override_boost || 0;
        if (isVhfCoreForOverride && overrideBoost > 0) {
            const firedIds = Array.isArray(overridesFired) ? overridesFired : [];
            const anyHigh = firedIds.some(id => CAL.high_confidence_override_ids.includes(id));
            const anyNonspecific = firedIds.some(id => CAL.nonspecific_override_ids.includes(id));
            if (anyNonspecific && !anyHigh) {
                const scale = CAL.nonspecific_override_vhf_boost_scale;
                const delta = Math.round(overrideBoost * (scale - 1)); // negative
                adjustments.override_boost_scale_delta = delta;
                adjustments.notes.push('nonspecific_override_vhf_damped');
            }
        }

        // Sum deltas
        const totalDelta =
            adjustments.absent_symptom_extra +
            adjustments.nonspecific_hemorrhage +
            adjustments.outbreak_plausibility +
            adjustments.syndrome_other +
            adjustments.override_boost_scale_delta;

        let score = r.final_score + totalDelta;

        // C3 — Incubation cap
        if (incState === false) {
            adjustments.incubation_cap_before = score;
            if (score > CAL.incubation_violation_cap) {
                score = CAL.incubation_violation_cap;
                adjustments.incubation_cap_applied = true;
                adjustments.notes.push('incubation_window_violated');
            }
        }

        score = Math.max(0, Math.min(100, Math.round(score)));

        // Supports (post-delta so uses final syndrome state)
        const supports = computeSupports(
            r, disease, presentSymptoms, selectedExposures, overridesFired,
            CAL.vhf_exposure_specific, incState, adjustments
        );
        adjustments.supports = supports;

        // C10 — VHF epidemiological-plausibility gate (calibration 3.2.0).
        //
        // A VHF core disease (Ebola, Marburg, Lassa, CCHF, RVF, Nipah,
        // Hantavirus) may only rank above the "low" band when at LEAST ONE
        // of the hard epidemiological signals is present:
        //
        //   a) Outbreak context — disease is flagged for a visited country
        //      by ENDEMIC_COUNTRIES (outbreak_bonus was actually applied).
        //   b) Direct hard exposure — contact with body fluids / dead body /
        //      funeral / healthcare / close-contact / lab — i.e. the kind of
        //      exposure that actually transmits a filovirus.
        //   c) Confirmed bleeding — true haemorrhagic signs (not epistaxis /
        //      gingival bleeding alone, which C2 already de-rates).
        //
        // Without any hard signal, the VHF is capped at "low" (39) so it
        // can't be the top candidate for a traveller with a generic febrile
        // illness. The underlying severity / priority tier is unchanged, so
        // a true VHF presentation still fires the override pathway and the
        // IHR flags as normal — this is pure specificity control.
        if (CAL.vhf_plausibility_enabled && CAL.vhf_core.includes(disease.id)) {
            const outbreakActive = (breakdown.outbreak_bonus || 0) > 0;
            const hardExposure = selectedExposures.some(e => CAL.vhf_hard_exposures.includes(e));
            const trueHemorrhage = presentSymptoms.some(s => CAL.vhf_true_hemorrhage_symptoms.includes(s));
            const plausible = outbreakActive || hardExposure || trueHemorrhage;
            if (!plausible && score > CAL.vhf_plausibility_cap) {
                if (!adjustments.notes.includes('vhf_plausibility_cap')) {
                    adjustments.notes.push('vhf_plausibility_cap');
                }
                adjustments.vhf_plausibility_cap_before = score;
                score = CAL.vhf_plausibility_cap;
            }
        }

        // C6 — Tier-1 / strict-Tier-1 promotion gate
        if (CAL.strict_tier1.includes(disease.id)) {
            // Always-notifiable IHR Tier-1 (smallpox, SARS, novel influenza,
            // polio). These must have supports >= min_supports_for_strict_tier1
            // or be capped below the TIER1_ALWAYS_CRITICAL escalation threshold.
            if (supports.total < CAL.min_supports_for_strict_tier1) {
                adjustments.promotion_cap_before = score;
                if (score > CAL.demotion_cap_unsupported) {
                    score = CAL.demotion_cap_unsupported;
                    adjustments.promotion_cap_applied = 'strict_tier1_insufficient';
                }
            }
        } else if (CAL.tier1_ihr_diseases.includes(disease.id)) {
            if (supports.total < CAL.min_supports_for_top_rank) {
                // Unsupported Tier-2-IHR disease: demote below the IHR
                // escalation threshold so it cannot force a CRITICAL alert.
                adjustments.promotion_cap_before = score;
                if (score > CAL.demotion_cap_unsupported) {
                    score = CAL.demotion_cap_unsupported;
                    adjustments.promotion_cap_applied = 'unsupported';
                }
            } else if (supports.total < CAL.min_supports_for_very_high_band) {
                adjustments.promotion_cap_before = score;
                if (score > CAL.demotion_cap_moderate) {
                    score = CAL.demotion_cap_moderate;
                    adjustments.promotion_cap_applied = 'moderate';
                }
            }
        }

        r.final_score = score;
        r.confidence_band = bandFor(score);
        r.calibration_adjustments = adjustments;
        return r;
    }

    // Wrap (now previously wrapped) scoreDiseases with the calibration pass.
    const _syndromeWrappedScore = D.scoreDiseases.bind(D);
    D.scoreDiseases = function (presentSymptoms, absentSymptoms, selectedExposures, context) {
        presentSymptoms = Array.isArray(presentSymptoms) ? presentSymptoms.slice() : [];
        absentSymptoms = Array.isArray(absentSymptoms) ? absentSymptoms.slice() : [];
        // UNKNOWN hygiene: a symptom recorded as both present AND absent is
        // contradictory — treat it as unknown by dropping it from both lists.
        const conflicts = presentSymptoms.filter(s => absentSymptoms.includes(s));
        if (conflicts.length > 0) {
            presentSymptoms = presentSymptoms.filter(s => !conflicts.includes(s));
            absentSymptoms = absentSymptoms.filter(s => !conflicts.includes(s));
        }

        selectedExposures = Array.isArray(selectedExposures) ? selectedExposures : [];
        context = context || {};

        const result = _syndromeWrappedScore(
            presentSymptoms, absentSymptoms, selectedExposures, context
        );

        const overridesFired = result.overrides_fired || [];
        const daysSinceOnset = (context.clinical_context || {}).days_since_onset;

        // Calibrate all top_diagnoses AND all_reportable so sorting is correct
        const diseaseById = id => D.diseases.find(d => d.id === id);
        const calibrateList = (list) => (list || []).map(r => {
            const disease = diseaseById(r.disease_id);
            if (!disease) return r;
            return calibrateOne(
                r, disease, presentSymptoms, absentSymptoms,
                selectedExposures, overridesFired, daysSinceOnset
            );
        });

        result.top_diagnoses = calibrateList(result.top_diagnoses);
        result.all_reportable = calibrateList(result.all_reportable);

        // Re-sort after calibration
        const bySeverity = (a, b) => {
            if (b.final_score !== a.final_score) return b.final_score - a.final_score;
            const ta = TIER_ORDER[a.priority_tier] ?? 9;
            const tb = TIER_ORDER[b.priority_tier] ?? 9;
            if (ta !== tb) return ta - tb;
            return (b.matched_hallmarks || []).length - (a.matched_hallmarks || []).length;
        };
        result.top_diagnoses.sort(bySeverity);
        result.all_reportable = result.all_reportable
            .filter(r => r.final_score >= 25)
            .sort(bySeverity);

        // Re-normalize probability_like_percent over the top 5.
        // Hard cap at 57% per executive policy — the deterministic engine
        // never claims certainty above that threshold.
        const top5 = result.top_diagnoses.slice(0, 5);
        const top5Total = top5.reduce((s, r) => s + r.final_score, 0);
        top5.forEach(r => {
            const raw = top5Total > 0
                ? 100 * r.final_score / top5Total
                : 0;
            r.probability_like_percent = Math.round(Math.min(raw, 57) * 10) / 10;
        });

        // Re-evaluate the post-scoring globalFlags that depend on Tier-1 position
        const globalFlags = new Set(result.global_flags || []);
        const tier1InTop3WithScore = top5.slice(0, 3).some(r =>
            ['tier_1_ihr_critical', 'tier_2_ihr_annex2'].includes(r.priority_tier) &&
            r.final_score >= 25
        );
        if (!tier1InTop3WithScore) {
            globalFlags.delete('NEEDS_PUBLIC_HEALTH_NOTIFICATION');
            // keep NEEDS_IHR_NOTIFICATION only if an override flag drove it
            if (!(overridesFired || []).some(id =>
                CAL.high_confidence_override_ids.includes(id))) {
                globalFlags.delete('NEEDS_IHR_NOTIFICATION');
            }
        }

        // Strip VHF protocol flags when the only VHF signal is non-specific.
        // When no VHF core disease in top_diagnoses has supports >= min_supports
        // AND no high-confidence VHF override (Rule 1 with exposure) fired,
        // VHF_PROTOCOL_ACTIVATED / NEEDS_IMMEDIATE_ISOLATION are unjustified.
        // This prevents IHR rule FEVER_HEMORRHAGE_EMERGENCY from firing on
        // epistaxis alone.
        const anyVhfSupported = (result.top_diagnoses || []).some(r => {
            const isVhf = CAL.vhf_core.includes(r.disease_id);
            const sup = r.calibration_adjustments
                && r.calibration_adjustments.supports
                ? r.calibration_adjustments.supports.total : 0;
            return isVhf && sup >= CAL.min_supports_for_top_rank;
        });
        const highConfVhfOverrideFired = (overridesFired || []).includes('override_vhf_red_flag');
        if (!anyVhfSupported && !highConfVhfOverrideFired) {
            globalFlags.delete('VHF_PROTOCOL_ACTIVATED');
            globalFlags.delete('NEEDS_IMMEDIATE_ISOLATION');
        }
        result.global_flags = Array.from(globalFlags);

        result.calibration = {
            version: CAL.version,
            unknown_conflicts_removed: conflicts,
        };

        return result;
    };

    // Wrap getEnhancedScoreResult for non-case fallback + officer override.
    const _baseGetEnhanced = D.getEnhancedScoreResult.bind(D);
    D.getEnhancedScoreResult = function (
        presentSymptoms, absentSymptoms, exposureEngineCodes,
        visitedCountries, vitals, context
    ) {
        presentSymptoms = Array.isArray(presentSymptoms) ? presentSymptoms.slice() : [];
        absentSymptoms = Array.isArray(absentSymptoms) ? absentSymptoms.slice() : [];
        ...
        // UNKNOWN hygiene at the top layer too — remove symptoms present in both.
        const conflicts = presentSymptoms.filter(s => absentSymptoms.includes(s));
        if (conflicts.length > 0) {
            presentSymptoms = presentSymptoms.filter(s => !conflicts.includes(s));
            absentSymptoms = absentSymptoms.filter(s => !conflicts.includes(s));
        ...
        const baseResult = _baseGetEnhanced(
            presentSymptoms, absentSymptoms, exposureEngineCodes,
            visitedCountries, vitals, context
        );

        const officerSuspicion = context.officer_declared_suspicion || null;
        const overridesFired = baseResult.overrides_fired || [];
        const topDx = (baseResult.top_diagnoses || [])[0] || null;
        const topScore = topDx ? topDx.final_score : 0;
        const topSupports = topDx && topDx.calibration_adjustments
            ? topDx.calibration_adjustments.supports : null;

        const highConfidenceOverrideFired = overridesFired.some(id =>
            CAL.high_confidence_override_ids.includes(id));

        // Only a high-confidence override that also has a qualifying exposure
        // should be allowed to BYPASS the non-case fallback.
        const anyVhfSpecificExposure = (exposureEngineCodes || []).some(e =>
            CAL.vhf_exposure_specific.includes(e));
        const overrideBypassesFallback = highConfidenceOverrideFired && anyVhfSpecificExposure;

        const shouldFallbackNonCase =
            !baseResult.is_non_case &&
            !overrideBypassesFallback &&
            topScore < CAL.non_case_fallback_top_score_threshold &&
            (!topSupports || topSupports.total < CAL.non_case_fallback_supports_threshold);

        if (shouldFallbackNonCase) {
            const flags = Array.from(new Set([
                ...(baseResult.global_flags || []),
                'NO_CURRENT_MATCH',
            ]));
            const preservedNonCase = baseResult.non_case || {};
            baseResult.is_non_case = true;
            baseResult.top_disease_id = null;
            baseResult.top_disease_confidence = null;
            baseResult.suggested_risk_level = baseResult.clinical_validation &&
                baseResult.clinical_validation.needs_emergency_triage ? 'HIGH' : 'LOW';
            baseResult.suggested_routing = 'DISTRICT';
            baseResult.ihr_notification_required = false;
            baseResult.global_flags = flags;
            baseResult.non_case = {
                ...preservedNonCase,
                isNonCase: true,
                reasons: [
                    ...(preservedNonCase.reasons || []),
                    'No disease cleared its WHO specificity gate. Top candidate score ' +
                    topScore + ' below fallback threshold ' +
                    CAL.non_case_fallback_top_score_threshold + '.',
                ],
                recommended_syndrome: baseResult.syndrome
                    ? baseResult.syndrome.syndrome : 'OTHER',
                recommended_disposition: 'NO_CURRENT_MATCH',
                can_override: true,
            };
            baseResult.ihr_risk = {
                risk_level: baseResult.suggested_risk_level,
                routing_level: 'DISTRICT',
                ihr_alert_required: false,
                ihr_tier: null,
                triggered_rules: [],
                reasoning: ['No disease cleared the specificity gate — "no current match" verdict.'],
            };
        }

        // C8 — Officer-declared suspicion raises the alert.
        // Accepts a string (disease ID) or an object with min_risk_level.
        // EXECUTIVE POLICY 2026-05-05: VHF declarations are gated by the
        // bleed-gate — no bleeding = no CRITICAL, no NATIONAL routing,
        // no TIER_1, no VHF_PROTOCOL. The declaration is still recorded
        // (officer judgement preserved) but at most HIGH / PHEOC.
        if (officerSuspicion) {
            const VHF_DISEASE_IDS_C8 = new Set([
                'ebola_virus_disease', 'marburg_virus_disease', 'lassa_fever',
                'cchf', 'rift_valley_fever', 'dengue_severe',
            ]);
            const C8_BLEEDING = new Set([
                'bleeding', 'bleeding_gums_or_nose', 'bloody_sputum',
            ...
            const suspicionId = typeof officerSuspicion === 'string' ? officerSuspicion
                              : (officerSuspicion.disease_id || null);
            const isVhfDisease = suspicionId && VHF_DISEASE_IDS_C8.has(suspicionId);
            const hasBleeding = (presentSymptoms || []).some(s => C8_BLEEDING.has(s));
            // Bleed-gate: VHF promotion only when bleeding is recorded.
            const vhfPromotionAllowed = isVhfDisease && hasBleeding;

            const minLevel = (typeof officerSuspicion === 'object' && officerSuspicion.min_risk_level)
                ? officerSuspicion.min_risk_level
                : (vhfPromotionAllowed ? 'CRITICAL' : 'HIGH');
            const levelRank = { LOW: 0, MEDIUM: 1, HIGH: 2, CRITICAL: 3 };
            const currentRank = levelRank[baseResult.ihr_risk?.risk_level] ?? 0;
            const wantedRank = levelRank[minLevel] ?? 2;
            const finalLevel = wantedRank > currentRank ? minLevel : baseResult.ihr_risk.risk_level;

            const routingLevel = vhfPromotionAllowed ? 'NATIONAL'
                : (wantedRank >= 3 ? 'NATIONAL'
                : (wantedRank >= 2 ? 'PHEOC'
                : ((baseResult.ihr_risk && baseResult.ihr_risk.routing_level) || 'DISTRICT')));

            const reasoningLine = vhfPromotionAllowed
                ? 'VHF disease declared by officer with recorded bleeding evidence — CRITICAL risk and NATIONAL routing are mandatory per IHR.'
                : (isVhfDisease
                    ? 'Officer declared a VHF disease but no bleeding sign is recorded. Per executive policy 2026-05-05, the VHF bleed-gate is unconditional — declaration recorded but VHF protocol NOT activated; alert capped at HIGH / PHEOC routing.'
                    : 'Officer-declared clinical suspicion — alert raised regardless of score.');

            baseResult.ihr_risk = {
                ...(baseResult.ihr_risk || {}),
                risk_level: finalLevel,
                routing_level: routingLevel,
                ihr_alert_required: true,
                ihr_tier: vhfPromotionAllowed ? 'TIER_1_ALWAYS_NOTIFIABLE' : (baseResult.ihr_risk?.ihr_tier || null),
                triggered_rules: [
                    ...((baseResult.ihr_risk && baseResult.ihr_risk.triggered_rules) || []),
                    'OFFICER_DECLARED_SUSPICION',
                ],
                reasoning: [
                    ...((baseResult.ihr_risk && baseResult.ihr_risk.reasoning) || []),
                    reasoningLine,
                ],
            };
            baseResult.suggested_risk_level = finalLevel;
            baseResult.suggested_routing = routingLevel;
            baseResult.ihr_notification_required = true;
            baseResult.is_non_case = false;
            baseResult.global_flags = Array.from(new Set([
                ...(baseResult.global_flags || []),
                'OFFICER_DECLARED_SUSPICION',
                ...(vhfPromotionAllowed ? ['VHF_PROTOCOL_ACTIVATED', 'NEEDS_IMMEDIATE_ISOLATION', 'NEEDS_IHR_NOTIFICATION'] : []),
                ...(isVhfDisease && !hasBleeding ? ['VHF_DECLARED_BUT_BLEED_GATE_CLOSED'] : []),
            ]));
            baseResult.officer_declared_suspicion = officerSuspicion;
        }

        baseResult.calibration_version = CAL.version;
        return baseResult;
    };

    // ══════════════════════════════════════════════════════════════════════
    // SECTION 11 — Executive guard rails (2026-05-05, revised)
    //
    // Final post-processor that enforces executive rules over the existing
    // scoring engine without replacing it:
    //
    //   R1. NO "NON-CASE" WHEN A SYMPTOM IS PRESENT
    //       If the officer ticked even one symptom, the engine MUST emit at
    //       least one suspected diagnosis. A symptom can never produce
    //       silence — that is operationally unsafe.
    //
    //   R2. EXACTLY ONE SUSPECTED DISEASE  (revised 2026-05-05)
    //       Engine output is capped to a single top-ranked disease. This
    //       matches the officer-override single-disease rule and prevents
    //       the alert template from listing 2-3 diseases it would treat
    //       with equal weight. Floor case is also single — the
    //       _alternativeFloor pair has been removed.
    //
    //   R3. VHF HARD BLEED-GATE  (revised 2026-05-05)
    //       Ebola, Marburg, Lassa, CCHF, RVF, Hantavirus, and Severe Dengue
    //       are excluded from EVERY output channel (top_diagnoses, IHR
    //       ladder, all_reportable, officer-override picker) unless a
    //       severe bleeding sign is present. The previous "full WHO VHF
    //       case definition" bypass and the "officer override" bypass have
    //       both been removed — bleeding is the only signal that opens the
    //       gate. An officer who declares a VHF without recorded bleeding
    //       has the declaration REJECTED with a global flag.
    //       Severe-bleeding signs: bleeding, bleeding_gums_or_nose,
    //       bloody_sputum, haematemesis, haematochezia, haematuria,
    //       haemorrhagic_skin_rash, petechial_or_purpuric_rash,
    //       bruising_or_ecchymosis, melena, bloody_diarrhea, blood_in_vomit.
    //
    //   R4. SKIN PRESENTATION → MPOX + MEASLES POOL  (revised 2026-05-05)
    //       Rash / vesicles / pustules / skin ulcer with no haemorrhagic
    //       sign restrict the candidate pool to Mpox + Measles. The
    //       previous mpox+measles auto-synthesis (which surfaced these
    //       diseases on every skin-only presentation even without scoring
    //       evidence — a known bias) has been removed. Filtering only;
    //       R1 floor-case applies if the pool is empty.
    //
    //   OFFICER OVERRIDE (revised 2026-05-05)
    //     • Single disease only (forced_included_ids array dropped).
    //     • Selectability gate via D.engine.getOfficerOverrideEligibility()
    //       — declarations missing minimum symptom/exposure evidence are
    //       REJECTED with a reason and required-evidence list.
    //     • Score breakdown via D.engine.buildOfficerOverrideExplanation()
    //       — deterministic factor-by-factor (which symptom contributed
    //       what weight, which exposure contributed what weight, the
    //       officer-override boost, and the final score) so the UI can
    //       render "why this score" rather than a black-box CRITICAL.
    // ══════════════════════════════════════════════════════════════════════
    ...
    const SKIN_PRESENT_SYMPTOMS = new Set([
        'rash_maculopapular', 'rash_vesicular_pustular', 'rash_petechial',
        'pustular_rash', 'skin_ulcer', 'skin_lesion', 'skin_eschar',
    ]);
    const SEVERE_BLEEDING_SYMPTOMS = new Set([
        'bleeding', 'bleeding_gums_or_nose', 'bloody_sputum',
        'haematemesis', 'haematochezia', 'haematuria',
        'haemorrhagic_skin_rash', 'petechial_or_purpuric_rash',
        'bruising_or_ecchymosis', 'melena',
    ]);
    const VHF_SUPPORTING_FEATURES = new Set([
        'severe_headache', 'muscle_pain', 'joint_pain', 'vomiting',
        'diarrhea', 'abdominal_pain', 'severe_fatigue', 'weakness',
    ]);
    const VHF_EPI_LINKS = new Set([
        'travel_from_outbreak_area', 'contact_confirmed_case',
        'contact_body_fluids', 'contact_with_sick_person',
        'healthcare_exposure', 'hcw_exposure',
        'bat_exposure', 'rodent_exposure', 'monkey_primate_exposure',
        'tick_bite', 'livestock_exposure', 'flood_livestock_exposure',
        'attended_funeral_outbreak_area',
    ]);

    function _hasFever(presentSymptoms, vitals) {
        if (presentSymptoms && presentSymptoms.includes('fever')) return true;
        const t = Number(vitals && (vitals.temperature_value ?? vitals.temperature ?? vitals.temp));
        return Number.isFinite(t) && t >= 38;
    }

    function _hasSevereBleeding(presentSymptoms) {
        if (!Array.isArray(presentSymptoms)) return false;
        return presentSymptoms.some(s => SEVERE_BLEEDING_SYMPTOMS.has(s));
    }

    function _hasSkinPresentation(presentSymptoms) {
        if (!Array.isArray(presentSymptoms)) return false;
        return presentSymptoms.some(s => SKIN_PRESENT_SYMPTOMS.has(s));
    }

    function _meetsFullVhfDefinition(presentSymptoms, exposures, vitals) {
        if (!_hasFever(presentSymptoms, vitals)) return false;
        const supportCount = (presentSymptoms || []).filter(s => VHF_SUPPORTING_FEATURES.has(s)).length;
        if (supportCount < 3) return false;
        const epiLinkInSymptoms = (presentSymptoms || []).some(s => VHF_EPI_LINKS.has(s));
        const epiLinkInExposures = (exposures || []).some(s => VHF_EPI_LINKS.has(s));
        return epiLinkInSymptoms || epiLinkInExposures;
    }

    function _vhfGateOpen(presentSymptoms, exposures, vitals) {
        // EXECUTIVE POLICY 2026-05-05 (unconditional bleed-gate):
        // VHF (Ebola, Marburg, Lassa, CCHF, RVF) is gated SOLELY on the
        // presence of a severe bleeding sign. No combination of fever,
        // exposure, full-WHO-definition triad, or officer override opens
        // the gate. If no bleeding symptom is recorded, no VHF appears
        // anywhere — top_diagnoses, IHR ladder, officer-override picker.
        return _hasSevereBleeding(presentSymptoms);
    }

    function _floorCase(presentSymptoms) {
        // Synthesised floor diagnosis when the engine returns nothing for a
        // symptomatic traveller. Maps the broad symptom set to the most
        // plausible benign label so we never emit silence.
        const has = id => (presentSymptoms || []).includes(id);
        let id, name;
        if (has('watery_diarrhea') || has('diarrhea') || has('vomiting')) {
            id = 'acute_gastroenteritis';
            name = 'Acute gastroenteritis';
        } else if (has('cough') || has('sore_throat') || has('coryza') || has('shortness_of_breath')) {
            id = 'upper_respiratory_tract_infection';
            name = 'Upper respiratory tract infection';
        } else {
            id = 'acute_febrile_illness';
        ...
        return {
            ...
            raw_score: 22,
            confidence_band: 'low',
            priority_tier: 'tier_4_syndromic',
            ihr_category: null,
            severity: 'mild',
            cfr_pct: null,
            alert_level: 'medium',
            matched_hallmarks: [],
            matched_symptoms: presentSymptoms || [],
            matched_exposures: [],
            score_breakdown: { gate_score: 0, symptom_score: 22, exposure_score: 0,
                syndrome_bonus: 0, outbreak_bonus: 0, vaccination_modifier: 0,
                onset_modifier: 0, absent_hallmark_penalty: 0, contradiction_penalty: 0,
                override_boost: 0 },
            key_distinguishers: [],
            recommended_tests: [],
            immediate_actions: ['Routine clinical assessment', 'Symptomatic care'],
            differential_diseases: [],
            probability_like_percent: 57,
            _is_floor_case: true,
        };
    }

    function _alternativeFloor(presentSymptoms, primary) {
        // Provide a safe second-pick when only one disease passes the floor —
        // executives review pairs more reliably than singletons.
        const id = primary && primary.disease_id;
        if (id !== 'acute_febrile_illness') {
            return {
                ...(_floorCase(presentSymptoms)),
                disease_id: 'acute_febrile_illness',
                name: 'Acute febrile illness',
                final_score: 18,
                raw_score: 18,
                probability_like_percent: 0,
                _is_floor_case: true,
            };
        }
        return {
            ..._floorCase(presentSymptoms),
            disease_id: 'viral_syndrome_unspecified',
            name: 'Unspecified viral syndrome',
            final_score: 16,
            raw_score: 16,
            probability_like_percent: 0,
            _is_floor_case: true,
    ...
    const _origGetEnhanced = D.getEnhancedScoreResult;
    D.getEnhancedScoreResult = function (
        presentSymptoms, absentSymptoms, exposureEngineCodes, visitedCountries, vitals, context
    ) {
        const present = Array.isArray(presentSymptoms) ? presentSymptoms : [];
        const exposures = Array.isArray(exposureEngineCodes) ? exposureEngineCodes : [];
        const visited = Array.isArray(visitedCountries) ? visitedCountries : [];
        const v = vitals || {};
        const ctx = context || {};

        // R1 — symptomatic traveller must always produce a diagnosis. Force
        // the scoring path even when getNonCaseVerdict() votes non-case.
        const hasAnySymptom = present.length > 0 || _hasFever(present, v);
        let result = _origGetEnhanced.call(this, present, absentSymptoms, exposures, visited, v, ctx);
        if (hasAnySymptom && result && result.is_non_case) {
            try {
                const D2 = D;
                const scoreResult = D2.scoreDiseases(present, absentSymptoms, exposures, {
                    outbreak_context: D2.buildOutbreakContext(visited),
                    clinical_context: {
                        ...v,
                        ...((v && v.clinical_context) || {}),
                        ...(ctx.clinical_context || {}),
                    },
                    vaccination_history: ctx.vaccination_history || {},
                });
                const ihrRisk = D2.computeIHRRiskLevel(scoreResult, v, scoreResult.global_flags);
                result = {
                    top_diagnoses: scoreResult.top_diagnoses || [],
                    all_reportable: scoreResult.all_reportable || [],
                    overrides_fired: scoreResult.overrides_fired || [],
                    global_flags: scoreResult.global_flags || [],
                    input_summary: scoreResult.input_summary || {},
                    syndrome: D2.deriveWHOSyndrome(present, v),
                    ihr_risk: ihrRisk,
                    non_case: { isNonCase: false, reasoning: 'Forced scoring — symptom present overrides non-case verdict.' },
                    clinical_validation: D2.validateClinicalData(present, v),
                    outbreak_context_used: D2.buildOutbreakContext(visited),
                    is_non_case: false,
                    ihr_notification_required: ihrRisk.ihr_alert_required,
                ...
                // If forced scoring throws, fall back to a single floor case
                // — never let R1 surface silence.
                console.debug('[guard-rails] forced scoring threw, using floor:', err && err.message);
                const f = _floorCase(present);
                result = {
                    top_diagnoses: [f, _alternativeFloor(present, f)],
                    all_reportable: [f],
                    overrides_fired: [],
                    global_flags: ['FLOOR_CASE_APPLIED'],
                    input_summary: { symptoms_count: present.length, confidence_baseline: 'floor' },
                    syndrome: { syndrome: 'NONE', confidence: 'LOW', reasoning: 'Floor case — engine error.', who_criteria_met: [] },
                    ihr_risk: { risk_level: 'LOW', routing_level: 'DISTRICT', ihr_alert_required: false, ihr_tier: null, triggered_rules: [], reasoning: ['Floor case applied'] },
                    non_case: { isNonCase: false, reasoning: 'Floor case' },
                    clinical_validation: { needs_emergency_triage: false },
                    outbreak_context_used: [],
                    is_non_case: false,
                    ihr_notification_required: false,
                    show_emergency_banner: false,
                    top_disease_id: f.disease_id,
                    top_disease_confidence: 'low',
                    suggested_risk_level: 'LOW',
                    suggested_routing: 'DISTRICT',
                };
            }
        ...
        let top = Array.isArray(result.top_diagnoses) ? result.top_diagnoses.slice() : [];
        // Keep a frozen copy of the original engine top-5 so the R2 padding
        // step (below) can fall back to lower-scoring candidates when the
        // VHF / skin-pool filters reduce the differential to <3 entries.
        const _engineTop5 = top.slice();

        // R3 — VHF bleed-gate (EXECUTIVE POLICY 2026-05-05).
        // Hard, unconditional rule: VHF (Ebola, Marburg, Lassa, CCHF, RVF,
        // Hantavirus, Severe Dengue) is filtered out of every output channel
        // unless a severe bleeding sign is present. Officer override does
        // NOT bypass — the bleed-gate is checked separately in the officer
        // override block below and rejects the declaration with a flag.
        const vhfGateOpen = _vhfGateOpen(present, exposures, v);
        if (!vhfGateOpen) {
            top = top.filter(d => !VHF_DISEASE_IDS.has(d.disease_id));
            if (Array.isArray(result.all_reportable)) {
                result.all_reportable = result.all_reportable.filter(d => !VHF_DISEASE_IDS.has(d.disease_id));
            }
        }

        // R4 — Skin-only presentation: filter pool to IDSR rash-presenting
        // diseases. The original mpox+measles-only pool was set when mpox was
        // an active engine entity; with mpox now legacy (I-4 forbids it from
        // top), the pool reduced to measles-only — which silently blocked
        // smallpox (the IDSR-correct top for vesicular pustular rash same-stage,
        // Annex 1A Tier 1) and other Annex-1A rash diseases. Pool now lists
        // every IDSR active disease whose case definition is rash-anchored.
        // mpox stays in the static pool for the legacy-display path; it
        // cannot reach top_diagnoses through scoring (I-4 enforced upstream).
        const skinPresent = _hasSkinPresentation(present);
        const bleeding   = _hasSevereBleeding(present);
        if (skinPresent && !bleeding) {
            const skinPool = new Set([
                'mpox', 'measles', 'rubella', 'smallpox',
                'anthrax_cutaneous', 'yaws', 'leprosy', 'onchocerciasis',
                'chikungunya', 'zika',
            ]);
            top = top.filter(d => skinPool.has(d.disease_id));
            if (Array.isArray(result.all_reportable)) {
                result.all_reportable = result.all_reportable.filter(d => skinPool.has(d.disease_id));
            }
        }

        // Officer override — single disease + bleed-gate + explainable score.
        // EXECUTIVE POLICY 2026-05-05:
        //   • Cap to ONE disease (was up to two via forced_included_ids).
        //   • VHF bleed-gate applies — declaration of Ebola, Marburg, Lassa,
        //     CCHF, RVF, Hantavirus, or Severe Dengue without a recorded
        //     bleeding sign is REJECTED with a global flag.
        //   • Selectability gate — declaration of any disease whose
        //     minimum symptom/exposure evidence is missing is REJECTED.
        //   • The forced entry carries a deterministic, factor-by-factor
        //     score breakdown (officer_explanation) so the UI can render
        //     "why this score" rather than a black-box CRITICAL.
        if (ctx.officer_override && ctx.officer_declared_suspicion) {
            const declaredId = typeof ctx.officer_declared_suspicion === 'string'
                ? ctx.officer_declared_suspicion : ctx.officer_declared_suspicion.disease_id;
            const catalogEntry = declaredId ? D.diseases.find(d => d.id === declaredId) : null;

            if (declaredId && catalogEntry) {
                const eligibility = D.engine && D.engine.getOfficerOverrideEligibility
                    ? D.engine.getOfficerOverrideEligibility(declaredId, present, exposures, v)
                    : { selectable: true, reason: '' };

                if (!eligibility.selectable) {
                    result.officer_override_rejected = {
                        disease_id: declaredId,
                        disease_name: catalogEntry.name || declaredId,
                        reason: eligibility.reason,
                        required_evidence: eligibility.required_evidence || null,
                    };
                    result.global_flags = Array.isArray(result.global_flags) ? result.global_flags : [];
                    const flagId = VHF_DISEASE_IDS.has(declaredId) && !_hasSevereBleeding(present)
                        ? 'OFFICER_OVERRIDE_REJECTED_VHF_NO_BLEEDING'
                        : 'OFFICER_OVERRIDE_REJECTED_INSUFFICIENT_EVIDENCE';
                    if (!result.global_flags.includes(flagId)) result.global_flags.push(flagId);
                } else {
                    const explanation = D.engine && D.engine.buildOfficerOverrideExplanation
                        ? D.engine.buildOfficerOverrideExplanation(declaredId, present, exposures, v)
                        : null;

                    const fromAll = (result.all_reportable || []).find(d => d.disease_id === declaredId);
                    const finalScore = explanation ? explanation.final_score : 50;
                    const confidenceBand = finalScore >= 70 ? 'high'
                                         : finalScore >= 50 ? 'moderate'
                                         : finalScore >= 30 ? 'low' : 'minimal';

                    const baseEntry = fromAll || {
                        disease_id:              declaredId,
                        name:                    catalogEntry.name || declaredId,
                        priority_tier:           catalogEntry.priority_tier || 'tier_2_ihr_annex2',
                        ihr_category:            catalogEntry.who_category || null,
                        severity:                catalogEntry.severity || 4,
                        cfr_pct:                 catalogEntry.case_fatality_rate_pct || null,
                        alert_level:             catalogEntry.alert_level_if_top_ranked || 'high',
                        matched_hallmarks:       explanation ? explanation.symptom_factors.map(f => f.symptom) : [],
                        matched_symptoms:        present,
                        matched_exposures:       exposures,
                        key_distinguishers:      catalogEntry.key_distinguishers || [],
                        recommended_tests:       catalogEntry.recommended_tests || [],
                        immediate_actions:       catalogEntry.immediate_actions || [],
                        differential_diseases:   [],
                        probability_like_percent: 0,
                    };
                    ...
                    const artificialEntry = {
                        ...
                        score_breakdown:    explanation
                            ? { gate_score: 0,
                                symptom_score: explanation.base_symptom_score,
                                exposure_score: explanation.base_exposure_score,
                                syndrome_bonus: 0, outbreak_bonus: 0,
                                vaccination_modifier: 0, onset_modifier: 0,
                                absent_hallmark_penalty: 0, contradiction_penalty: 0,
                                override_boost: explanation.override_boost }
                            : (baseEntry.score_breakdown || {}),
                        _officer_forced:    true,
                        officer_explanation: explanation,
                    };

                    // Single officer-declared diagnosis replaces the engine's pool.
                    top = [artificialEntry];
                    result.officer_override_accepted = {
                        disease_id: declaredId,
                        disease_name: catalogEntry.name || declaredId,
        ...
        // R1 (continued) — if the gate-filtered pool is empty but the
        // traveller is symptomatic, insert the floor case so the alert
        // template never sees zero suspected diseases. Single floor only
        // (executive policy 2026-05-05 — no _alternativeFloor pair).
        if (hasAnySymptom && top.length === 0) {
            const floor = _floorCase(present);
            top = [floor];
            result.global_flags = Array.isArray(result.global_flags) ? result.global_flags : [];
            if (!result.global_flags.includes('FLOOR_CASE_APPLIED')) result.global_flags.push('FLOOR_CASE_APPLIED');
        }

        // R2 — top-3 differential (revised 2026-05-08). Engine surfaces
        // up to three ranked, DISTINCT candidates with DISTINCT names and
        // DISTINCT scores so the officer reviews a real differential.
        //
        // Step 1: dedup the AHF / VHF family. diseaseDisplayName() collapses
        //         all 4 AHF disease IDs to "Viral Haemorrhagic Fever (VHF)"
        //         by clinical-policy choice (avoid panic before lab
        //         confirmation). Without dedup the officer sees identical
        //         "VHF" rows. Keep only the highest-scoring AHF representative.
        const _AHF_IDS = new Set(['ebola_virus_disease','marburg_virus_disease','lassa_fever','cchf']);
        const _familyKey = (id) => _AHF_IDS.has(id) ? '__AHF__' : id;
        const _seenFamilies = new Set();
        top = top.filter(r => {
            const k = _familyKey(r.disease_id);
            if (_seenFamilies.has(k)) return false;
            _seenFamilies.add(k);
            return true;
        });

        // Step 2: pad to 3 distinct entries when R3/R4 filters leave fewer.
        //         First try result.all_reportable (>=25 score), then fall
        //         back to the engine's original top-5 (which can include
        //         lower-scoring diseases). Always honour the AHF family
        //         dedup and skip any VHF-family entry when the bleed-gate
        //         is closed (vhfGateOpen=false).
        const _padFrom = (pool) => {
            for (const candidate of (pool || [])) {
                if (top.length >= 3) return;
                if (!candidate || !candidate.disease_id) continue;
                const k = _familyKey(candidate.disease_id);
                if (_seenFamilies.has(k)) continue;
                if (!vhfGateOpen && VHF_DISEASE_IDS.has(candidate.disease_id)) continue;
                _seenFamilies.add(k);
                top.push(candidate);
            }
        };
        if (top.length < 3) _padFrom(result.all_reportable);
        if (top.length < 3) _padFrom(_engineTop5);
        if (top.length > 3) top = top.slice(0, 3);

        // Step 3: tiebreaker — when two retained entries share an identical
        //         final_score (very common when diseases share the same
        //         hallmark weights), nudge later entries down by 1 each so
        //         scores are visibly distinct. The raw_score is preserved
        //         on each entry for the audit trail; only the displayed
        //         final_score is differentiated.
        for (let i = 1; i < top.length; i++) {
            if (top[i].final_score >= top[i - 1].final_score) {
                top[i].final_score = Math.max(0, top[i - 1].final_score - 1);
            }
        }

        // Step 4: re-normalise probability_like_percent over the deduped 3.
        //         Hard cap at 57 % per executive policy.
        const total = top.reduce((s, r) => s + (Number(r.final_score) || 0), 0);
        top.forEach(r => {
            const raw = total > 0
                ? 100 * (Number(r.final_score) || 0) / total
                : 0;
            r.probability_like_percent = Math.round(Math.min(raw, 57) * 10) / 10;
        });

        result.top_diagnoses = top;
        result.is_non_case = !hasAnySymptom && top.length === 0;
        result.top_disease_id = (top[0] || {}).disease_id || null;
        result.top_disease_confidence = (top[0] || {}).confidence_band || null;
        result.executive_guard_rails = {
            applied:        true,
            vhf_gate_open:  vhfGateOpen,
            skin_only_pool: skinPresent && !bleeding,
            output_capped:  true,
        };

        // ── Protocol-banner tie-down (executive directive 2026-05-08) ──
        // Triage overrides historically fired protocol flags (VHF,
        // CHOLERA, RABIES, BIOTERRORISM, …) based on raw symptom triggers.
        // After R3 (VHF bleed-gate), R4 (skin-pool filter), R2 (top-3 dedup)
        // and the officer override paths, the FINAL top_diagnoses may no
        // longer include the disease that originally triggered the flag —
        // yet the flag stays in result.global_flags, and the UI still
        // renders the corresponding red banner. That is the bug the user
        // reported: "VHF / RABIES / CHOLERA / BIOTERRORISM banners trigger
        // for diseases not in the differential."
        //
        // Fix: filter result.global_flags so each protocol flag survives
        // ONLY when at least one disease it points to is in the final
        // top_diagnoses. Generic flags (NEEDS_EMERGENCY_REFERRAL,
        // NEEDS_IHR_NOTIFICATION, NEEDS_IMMEDIATE_ISOLATION, etc.) are
        // tied to disease severity, not specific pathogens — those are
        // also conditioned on a CRITICAL/HIGH disease being present.
        const _topIds = new Set(top.map(d => d.disease_id));
        const _topAlertLevels = new Set(top.map(d => d.alert_level || ''));
        const _hasCriticalOrHigh = _topAlertLevels.has('critical') || _topAlertLevels.has('high');
        const _intersect = (ids) => ids.some(id => _topIds.has(id));
        const _PROTOCOL_FLAG_TIES = {
            VHF_PROTOCOL_ACTIVATED:        ['ebola_virus_disease','marburg_virus_disease','lassa_fever','cchf','rift_valley_fever','dengue_severe'],
            CHOLERA_PROTOCOL_ACTIVATED:    ['cholera'],
            RABIES_PROTOCOL_ACTIVATED:     ['rabies','dog_bite_rabies_exposure'],
            BIOTERRORISM_PROTOCOL_ACTIVATED: ['smallpox','anthrax_cutaneous','anthrax_pulmonary'],
            BIOTERRORISM_ASSESSMENT_REQUIRED: ['anthrax_cutaneous','anthrax_pulmonary','smallpox'],
            MENINGITIS_PROTOCOL_ACTIVATED: ['meningococcal_meningitis'],
            AFP_SURVEILLANCE_ACTIVATED:    ['polio'],
            PREGNANCY_RISK_FLAG:           ['zika','rubella','malaria_severe','malaria_uncomplicated','rift_valley_fever'],
        };
        const _GENERIC_NEEDS_CRITICAL_HIGH = new Set([
            'NEEDS_IMMEDIATE_ISOLATION',
            'NEEDS_EMERGENCY_REFERRAL',
            'NEEDS_PUBLIC_HEALTH_NOTIFICATION',
            'NEEDS_IHR_NOTIFICATION',
        ]);
        result.global_flags = (result.global_flags || []).filter(flag => {
            const tie = _PROTOCOL_FLAG_TIES[flag];
            if (tie) return _intersect(tie);
            if (_GENERIC_NEEDS_CRITICAL_HIGH.has(flag)) return _hasCriticalOrHigh;
            return true; // unknown / informational flags pass through
        });
    ...
    // ══════════════════════════════════════════════════════════════════════
    // SECTION 12 — Officer override APIs (executive policy 2026-05-05)
    //
    // Two pure functions exposed on D.engine for the Vue layer to call
    // BEFORE submitting an officer override:
    //
    //   getOfficerOverrideEligibility(diseaseId, present, exposureCodes, vitals)
    //     → { selectable, reason, required_evidence }
    //
    //     The picker uses this to disable impossible diseases (e.g.
    //     cholera without watery diarrhoea, VHF without bleeding,
    //     mpox without rash) and surface a tooltip explaining what
    //     evidence is missing.
    //
    //   buildOfficerOverrideExplanation(diseaseId, present, exposureCodes, vitals)
    //     → { symptom_factors, exposure_factors, base_symptom_score,
    //         base_exposure_score, override_boost, final_score, rationale }
    //
    //     The post-override view uses this to render a deterministic,
    //     factor-by-factor breakdown of how the score was arrived at —
    //     not a black-box CRITICAL.
    // ══════════════════════════════════════════════════════════════════════

    const VHF_BLEED_GATED_IDS = new Set([
        'ebola_virus_disease', 'marburg_virus_disease', 'lassa_fever',
        'cchf', 'rift_valley_fever', 'dengue_severe',
    ]);

    function _bleedingSymptomList() {
        return ['bleeding', 'bleeding_gums_or_nose', 'bloody_sputum',
                'haematemesis', 'haematochezia', 'haematuria',
                'haemorrhagic_skin_rash', 'petechial_or_purpuric_rash',
                'bruising_or_ecchymosis', 'melena',
                'bloody_diarrhea', 'blood_in_vomit'];
    ...
    D.engine.getOfficerOverrideEligibility = function (diseaseId, presentSymptoms, exposureCodes, vitals) {
        const disease = D.diseases.find(d => d.id === diseaseId);
        if (!disease) {
            return {
                selectable: false,
                reason: 'Unknown disease ID: ' + diseaseId + '.',
                required_evidence: { symptoms: [], exposures: [] },
            };
        }

        const present = Array.isArray(presentSymptoms) ? presentSymptoms : [];
        const expCodes = Array.isArray(exposureCodes) ? exposureCodes : [];

        // VHF hard bleed-gate — bleeding is the only signal that opens it.
        if (VHF_BLEED_GATED_IDS.has(diseaseId) && !_hasSevereBleeding(present)) {
            return {
                selectable: false,
                reason: (disease.name || diseaseId) + ' is a haemorrhagic-fever disease and cannot be declared without a recorded bleeding sign. Per WHO IHR + executive policy 2026-05-05, the bleed-gate is unconditional — exposure history, fever, or clinical suspicion alone do not satisfy it. Re-record any haemorrhagic sign (frank bleeding, bloody diarrhoea, petechial rash, etc.) on Step 2 before re-attempting.',
                required_evidence: {
                    symptoms: _bleedingSymptomList(),
                    exposures: [],
                },
            };
        }

        // Per-disease gate from catalog (gates.required_any).
        const gates = disease.gates || {};
        const required = Array.isArray(gates.required_any) ? gates.required_any : null;
        const sw = disease.symptom_weights || {};
        const ew = disease.exposure_weights || {};

        if (required && required.length > 0) {
            const allSignals = new Set([...present, ...expCodes]);
            const satisfied = required.some(r => allSignals.has(r));
            if (!satisfied) {
                const symList = required.filter(r => Object.prototype.hasOwnProperty.call(sw, r));
                const expList = required.filter(r => Object.prototype.hasOwnProperty.call(ew, r));
                return {
                    selectable: false,
                    reason: (disease.name || diseaseId) + ' requires at least one of: ' +
                            required.join(', ') +
                            '. None of these are recorded for this traveller — the declaration is not supported by the entered evidence.',
                    required_evidence: {
                        symptoms: symList.length ? symList : required,
                        exposures: expList,
                    },
                };
            }
        ...
        // Generic fallback — at least one strong (>=14 wt) signal must be present.
        const STRONG_WEIGHT = 14;
        const presentStrong = present.filter(s => (sw[s] || 0) >= STRONG_WEIGHT);
        const exposureStrong = expCodes.filter(e => (ew[e] || 0) >= STRONG_WEIGHT);
        if (presentStrong.length === 0 && exposureStrong.length === 0) {
            const sampleSymptoms = Object.keys(sw).filter(k => sw[k] >= STRONG_WEIGHT).slice(0, 6);
            const sampleExposures = Object.keys(ew).filter(k => ew[k] >= STRONG_WEIGHT).slice(0, 4);
            return {
                selectable: false,
                reason: (disease.name || diseaseId) + ' has no high-weight signal recorded for this traveller. Expected at least one of: ' +
                        [...sampleSymptoms, ...sampleExposures].slice(0, 8).join(', ') + '.',
                required_evidence: {
        ...
        return { selectable: true, reason: '', required_evidence: null };
    };

    D.engine.buildOfficerOverrideExplanation = function (diseaseId, presentSymptoms, exposureCodes, vitals) {
        const disease = D.diseases.find(d => d.id === diseaseId);
        if (!disease) return null;
        ...
        const symptomFactors = present
            .filter(s => (sw[s] || 0) > 0)
            .map(s => ({ symptom: s, weight: sw[s] }))
            .sort((a, b) => b.weight - a.weight);

        const exposureFactors = expCodes
            .filter(e => (ew[e] || 0) > 0)
            .map(e => ({ exposure: e, weight: ew[e] }))
            .sort((a, b) => b.weight - a.weight);

        const baseSymptomScore = symptomFactors.reduce((s, f) => s + f.weight, 0);
        const baseExposureScore = exposureFactors.reduce((s, f) => s + f.weight, 0);
        const overrideBoost = 30;
        const finalScore = Math.min(100, Math.max(0,
            baseSymptomScore + baseExposureScore + overrideBoost));

        const band = finalScore >= 70 ? 'high'
                   : finalScore >= 50 ? 'moderate'
                   : finalScore >= 30 ? 'low' : 'minimal';

        const rationale = [];
        rationale.push('Officer-declared suspicion: ' + (disease.name || diseaseId) + '.');
        if (symptomFactors.length > 0) {
            rationale.push('Symptoms scoring for this disease (' + baseSymptomScore + ' pts total): ' +
                symptomFactors.map(f => f.symptom + ' +' + f.weight).join(', ') + '.');
        } else {
            rationale.push('No recorded symptoms carry weight in this disease\'s catalog (0 pts).');
        }
        if (exposureFactors.length > 0) {
            rationale.push('Exposures scoring for this disease (' + baseExposureScore + ' pts total): ' +
                exposureFactors.map(f => f.exposure + ' +' + f.weight).join(', ') + '.');
        } else {
            rationale.push('No recorded exposures carry weight in this disease\'s catalog (0 pts).');
        }
        rationale.push('Officer-override boost: +' + overrideBoost + ' pts.');
        rationale.push('Final score: ' + finalScore + '/100 (band: ' + band + ').');

        return {
            disease_id: diseaseId,
            disease_name: disease.name || diseaseId,
            symptom_factors: symptomFactors,
            exposure_factors: exposureFactors,
            base_symptom_score: baseSymptomScore,
            base_exposure_score: baseExposureScore,
    ...
    console.log(
        '%c[WHO-POE Intelligence Layer] Loaded — ' + D.diseases.length + ' diseases enriched with endemic countries and case definitions. 9 API functions available. buildClinicalReport() generates complete WHO/IHR clinical intelligence reports. Calibration v' + CAL.version + ' active. Executive guard rails 2026-05-05 active (R1/R2/R3/R4 + officer override v2).',
