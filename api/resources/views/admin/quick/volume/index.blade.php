...
@section('content')
<div x-data="qrVolume()" x-init="boot()" class="qr-stack" :aria-busy="loading ? 'true' : 'false'">
    ...
    <section class="qr-card">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-3 p-4">
            ...
            <div><label class="label block mb-1 text-[11px]">Age band</label>
                <select class="select" x-model="filters.age_band"><option value="">All</option>
                    <template x-for="b in (payload.meta?.age_bands || [])" :key="b"><option :value="b" x-text="b"></option></template>
                </select>
            </div>
    ...
    <section class="qr-card">
        <div class="qr-card-head">
            <div class="min-w-0"><h2 class="qr-card-title" x-text="payload.chart?.title || 'Screening volume'"></h2><p class="qr-card-sub" x-text="payload.chart?.subtitle || ''"></p></div>
        ...
        <div class="p-4">
            ...
            <div class="qr-chart-wrap" x-show="ready && chartHasData" x-cloak><canvas x-ref="chart" role="img" aria-label="Screening volume"></canvas></div>
    ...
    <section class="qr-card overflow-hidden">
        <div class="qr-card-head">
            <div class="min-w-0"><h2 class="qr-card-title">Per-POE register</h2><p class="qr-card-sub" x-text="tableSub()"></p></div>
                ...
                <tbody>
                    ...
                    <template x-for="row in (payload.table || [])" :key="row.poe_code">
                        <tr>
                            ...
                            <td class="text-right pr-3 tabular-nums" x-text="row.median_age != null ? row.median_age : '—'" :class="row.median_age != null ? '' : 'qr-cell-mono'"></td>
                            <td x-text="row.last_screening_label || 'No activity'" :class="row.last_screening_label ? 'qr-cell-primary' : 'qr-cell-mono'"></td>
                        </tr>
                    </template>
...
<script>
    const QR_VOLUME = {
        endpointData:   @json(url('/admin/quick-reports/screening-volume/data')),
        endpointExport: @json(url('/admin/quick-reports/screening-volume/export')),
    };
    function qrVolume() {
        return {
            ready: false, loading: false, chartHasData: false, explainOpen: false, chart: null, lastLoadedAt: null, abortCtrl: null,
            filters: { days: '7', poe: '', gender: '', age_band: '', q: '' },
            payload: { window: null, scope: null, kpis: {}, chart: { labels: [], values: [] }, table: [], total_rows: 0, shown_rows: 0, meta: {} },
            boot() { this.readFiltersFromUrl(); this.loadData(); for (const k of ['days','poe','gender','age_band']) this.$watch(`filters.${k}`, () => this.loadData()); if (typeof Chart === 'undefined') { const w = setInterval(() => { if (typeof Chart !== 'undefined') { clearInterval(w); this.renderChart(); } }, 60); } },
            ...
            async loadData() {
                ...
                this.loading = true;
                try {
                    const res = await fetch(`${QR_VOLUME.endpointData}?${p.toString()}`, { headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin', signal: this.abortCtrl.signal });
                ...
                } catch (e) { if (e.name !== 'AbortError') { console.error('[qr-volume]', e); this.ready = true; } }
                finally { this.loading = false; }
            },
            resetFilters() { this.filters = { days: '7', poe: '', gender: '', age_band: '', q: '' }; this.loadData(); },
            ...
            exportChartPng() { if (!this.chart) return; const a = document.createElement('a'); a.href = this.chart.toBase64Image('image/png', 1.0); a.download = `qr-volume-chart-${this.fileStamp()}.png`; document.body.appendChild(a); a.click(); a.remove(); },
            exportChartCsv() { const labels = this.payload?.chart?.labels || []; const values = this.payload?.chart?.values || []; const lines = ['Bucket,Count']; for (let i = 0; i < labels.length; i++) lines.push(`"${String(labels[i]).replace(/"/g,'""')}",${values[i] ?? 0}`); const blob = new Blob(['﻿' + lines.join('\r\n')], { type: 'text/csv;charset=utf-8' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `qr-volume-chart-${this.fileStamp()}.csv`; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(a.href), 60_000); },
            exportCsvFull() { const p = new URLSearchParams(); for (const [k, v] of Object.entries(this.filters)) if (v !== '' && v != null) p.set(k, v); p.set('format', 'CSV'); window.location.href = `${QR_VOLUME.endpointExport}?${p.toString()}`; },
