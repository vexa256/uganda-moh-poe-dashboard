...
@section('content')
<div x-data="qrAlertDb()" x-init="boot()" class="qr-stack" :aria-busy="loading ? 'true' : 'false'">

    <div class="qr-progress" x-show="loading" x-cloak></div>

    <section class="flex flex-col sm:flex-row sm:items-end gap-3">
        <div class="min-w-0">
            <p class="eyebrow">Quick Reports · qr-alert-db</p>
            <h1 class="text-[20px] font-semibold tracking-tight">Alert Database</h1>
            <p class="help-text mt-1" x-text="headline()">Every alert in the window — who, where, severity, owner.</p>
        </div>
    ...
    <section class="qr-card">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 p-4">
            <div>
                <label class="label block mb-1 text-[11px]">Window</label>
                <select class="select" x-model="filters.days">
                    <option value="7">Past 7 days</option><option value="14">Past 14 days</option>
                    <option value="30">Past 30 days</option><option value="60">Past 60 days</option>
                    <option value="90">Past 90 days</option>
                </select>
            </div>
            <div>
                <label class="label block mb-1 text-[11px]">Point of entry</label>
            ...
            <div>
                <label class="label block mb-1 text-[11px]">Risk level</label>
                <select class="select" x-model="filters.risk">
                    <option value="">All risk levels</option>
                    <template x-for="r in (payload.meta?.risks || [])" :key="r"><option :value="r" x-text="prettyRisk(r)"></option></template>
                </select>
            </div>
            <div>
                <label class="label block mb-1 text-[11px]">Status</label>
                <select class="select" x-model="filters.status">
                    <option value="">All statuses</option>
                    <template x-for="s in (payload.meta?.statuses || [])" :key="s"><option :value="s" x-text="prettyStatus(s)"></option></template>
                </select>
            </div>
            <div>
                <label class="label block mb-1 text-[11px]">Owner</label>
                <select class="select" x-model="filters.owner">
                    <option value="">All owners</option>
                    <template x-for="(name, id) in (payload.meta?.owners || {})" :key="id"><option :value="id" x-text="name"></option></template>
                </select>
            </div>
            <div>
                <label class="label block mb-1 text-[11px]">Search</label>
                <input type="text" class="select" placeholder="Traveller, code, title…" x-model="filters.q" @keydown.enter.prevent="loadData()">
            </div>
        </div>
    </section>
    ...
    <section>
        ...
        <div class="qr-kpi-grid" x-show="ready" x-cloak>
            ...
            <div class="qr-kpi" :class="(payload.kpis?.open ?? 0) > 0 && 'qr-kpi-warn'"><p class="qr-kpi-label">Open</p><p class="qr-kpi-value" x-text="fmt(payload.kpis?.open)"></p><p class="qr-kpi-hint">No-one has acknowledged yet</p></div>
            <div class="qr-kpi"><p class="qr-kpi-label">Acknowledged</p><p class="qr-kpi-value" x-text="fmt(payload.kpis?.acknowledged)"></p><p class="qr-kpi-hint">Picked up by an officer</p></div>
            <div class="qr-kpi qr-kpi-info"><p class="qr-kpi-label">Closed</p><p class="qr-kpi-value" x-text="fmt(payload.kpis?.closed)"></p><p class="qr-kpi-hint">Resolved or ruled out</p></div>
    ...
    <section class="qr-card" aria-label="Alert distribution chart">
        <div class="qr-card-head">
            <div class="min-w-0">
                <h2 class="qr-card-title" x-text="payload.chart?.title || 'Alert distribution'"></h2>
        ...
        <div class="p-4">
            ...
            <div class="qr-chart-wrap" x-show="ready && chartHasData" x-cloak>
                <canvas x-ref="chart" role="img" aria-label="Alert distribution"></canvas>
            </div>
    ...
    <section class="qr-card overflow-hidden" aria-label="Alert register">
        <div class="qr-card-head">
            <div class="min-w-0">
                <h2 class="qr-card-title">Alert register</h2>
        ...
        <div class="qr-table-wrap">
            <table class="qr-table">
                <thead>
                    <tr>
                        <th>Opened</th><th>Alert</th><th>Traveller</th>
                        <th>Risk</th><th>IHR</th><th>Status</th>
                        <th>Owner</th><th>Point of entry</th>
                    ...
                    <template x-for="row in (payload.table || [])" :key="row.alert_id">
                        <tr>
                            <td>
                                ...
                                <div class="qr-cell-secondary" x-text="row.age_label"></div>
                            </td>
                            <td>
                            ...
                            <td><span class="qr-pill qr-pill-info" x-show="row.ihr_tier" x-text="'T' + row.ihr_tier"></span><span x-show="!row.ihr_tier" class="qr-cell-mono">—</span></td>
                ...
                <div class="qr-modal" role="dialog" aria-modal="true">
                    <div class="qr-modal-head">
                        <div class="min-w-0">
                            ...
                            <h3 class="text-[15px] font-semibold mt-0.5 truncate" x-text="payload.chart?.title || 'Alert distribution'"></h3>
                        </div>
                                ...
                                <table class="qr-table">
                                    ...
                                    <tbody>
                                        <template x-for="(lbl, i) in (payload.chart?.labels || [])" :key="i">
                                            <tr><td x-text="lbl"></td><td class="text-right pr-3 tabular-nums" x-text="payload.chart.values[i]"></td><td class="text-right pr-3 tabular-nums text-muted-foreground" x-text="explainPct(i)"></td></tr>
                                        </template>
...
<script>
    const QR_AD = {
        endpointData:   @json(url('/admin/quick-reports/alert-database/data')),
        endpointExport: @json(url('/admin/quick-reports/alert-database/export')),
    ...
    function qrAlertDb() {
        return {
            ...
            filters: { days: '7', poe: '', risk: '', status: '', owner: '', q: '' },
            payload: { window: null, scope: null, kpis: {}, chart: { labels: [], values: [] }, table: [], total_rows: 0, shown_rows: 0, meta: {} },

            boot() {
                this.readFiltersFromUrl();
                this.loadData();
                for (const k of ['days','poe','risk','status','owner']) this.$watch(`filters.${k}`, () => this.loadData());
                if (typeof Chart === 'undefined') {
                    const w = setInterval(() => { if (typeof Chart !== 'undefined') { clearInterval(w); this.renderChart(); } }, 60);
                }
            },
            readFiltersFromUrl() {
                const u = new URL(window.location.href);
                for (const k of Object.keys(this.filters)) { const v = u.searchParams.get(k); if (v !== null) this.filters[k] = v; }
            },
            writeFiltersToUrl() {
                const u = new URL(window.location.href);
                for (const [k, v] of Object.entries(this.filters)) { v === '' || v == null ? u.searchParams.delete(k) : u.searchParams.set(k, v); }
            ...
            async loadData() {
                ...
                this.loading = true;
                try {
                    const res = await fetch(`${QR_AD.endpointData}?${p.toString()}`, { headers: { Accept: 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin', signal: this.abortCtrl.signal });
                ...
                } catch (e) { if (e.name !== 'AbortError') { console.error('[qr-alert-db]', e); this.ready = true; } }
                finally { this.loading = false; }
            },
            resetFilters() { this.filters = { days: '7', poe: '', risk: '', status: '', owner: '', q: '' }; this.loadData(); },
            ...
            renderChart() {
                ...
                if (this.chart) {
                    this.chart.data.labels = labels; this.chart.data.datasets[0].data = values;
                    this.chart.data.datasets[0].backgroundColor = fill; this.chart.data.datasets[0].borderColor = border;
                ...
                this.chart = new Chart(this.$refs.chart.getContext('2d'), {
                    ...
                    options: {
                        indexAxis: 'y', responsive: true, maintainAspectRatio: false, animation: { duration: 260 },
                        ...
                        plugins: {
                            ...
                            tooltip: { backgroundColor: '#0F172A', titleFont: { weight: '600', size: 12 }, bodyFont: { size: 11.5 }, padding: 10, cornerRadius: 6,
            ...
            prettyStatus(s) { return QR_AD.statusLabels[s] || (s || '—'); },
            prettyRisk(r)   { return QR_AD.riskLabels[r]   || (r || '—'); },
            riskPill(r) {
                const b = 'qr-pill';
                if (r === 'CRITICAL') return b + ' qr-pill-crit';
                if (r === 'HIGH')     return b + ' qr-pill-high';
                if (r === 'MEDIUM')   return b + ' qr-pill-med';
                if (r === 'LOW')      return b + ' qr-pill-low';
                return b + ' qr-pill-muted';
            },
            statusPill(s) {
                const b = 'qr-pill';
                if (s === 'OPEN')         return b + ' qr-pill-high';
                if (s === 'ACKNOWLEDGED') return b + ' qr-pill-med';
                if (s === 'IN_PROGRESS')  return b + ' qr-pill-info';
                if (s === 'CLOSED')       return b + ' qr-pill-success';
                if (s === 'REOPENED')     return b + ' qr-pill-crit';
                return b + ' qr-pill-muted';
            },
            rowDemographics(row) {
                const bits = [];
            ...
            headline() {
                if (!this.ready) return 'Loading alerts…';
                const t = this.payload.total_rows ?? 0;
                const scope = this.payload.scope?.label || '';
                if (t === 0) return `No alerts${scope ? ' in ' + scope : ''}.`;
                return `${t.toLocaleString()} ${t === 1 ? 'alert' : 'alerts'}${scope ? ' · ' + scope : ''}`;
            },
            tableSub() {
                if (!this.ready) return '';
                const t = this.payload.total_rows ?? 0, s = this.payload.shown_rows ?? 0;
            ...
            filtersSummary() {
                ...
                if (f.risk)   bits.push(`risk ${f.risk.toLowerCase()}`);
            ...
            explainWhat() {
                ...
                switch (this.payload.chart?.kind) {
                    case 'risk':   return `How alerts opened in ${w} break down by clinical severity tier.`;
                    case 'status': return `Where the alerts opened in ${w} sit in the lifecycle right now.`;
                    case 'poe':    return `Which entry points produced alerts in ${w}.`;
            ...
            explainHowToRead() {
                switch (this.payload.chart?.kind) {
                    case 'risk':   return 'Bar colour matches the urgency — green = Low, deep orange = High, red = Critical, blue-grey = Not set.';
                    case 'status': return 'Bars are sorted from active (Open) to terminal (Closed). Reopened gets purple so it stands out.';
                    case 'poe':    return 'Bars are sorted from busiest to quietest.';
                    case 'day':    return 'Tall bars mark cluster days.';
                    default:       return '';
                }
            },
            explainNext() {
                switch (this.payload.chart?.kind) {
                    case 'risk':   return 'Critical + High demand same-day acknowledgement. Click any row to open the case file.';
                    case 'status': return 'Open alerts older than 24 h need supervisor attention.';
                    case 'poe':    return 'Spikes at one POE warrant staffing review.';
                    case 'day':    return 'Sudden day-on-day jumps often precede a confirmed cluster.';
                    default:       return '';
                }
            },
            explainPct(i) {
            ...
            exportChartPng() {
                ...
                a.download = `qr-alert-db-chart-${this.fileStamp()}.png`;
            ...
            exportChartCsv() {
                ...
                a.download = `qr-alert-db-chart-${this.fileStamp()}.csv`;
                document.body.appendChild(a); a.click(); a.remove();
            ...
            exportCsvFull() {
                ...
                for (const [k, v] of Object.entries(this.filters)) if (v !== '' && v != null) p.set(k, v);
                p.set('format', 'CSV');
                window.location.href = `${QR_AD.endpointExport}?${p.toString()}`;
            },
