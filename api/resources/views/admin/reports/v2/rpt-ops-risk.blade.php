...
<div x-data="rptOpsRisk()" x-init="boot()" class="space-y-4">
    ...
    <div class="flex items-center gap-1 border-b border-border/60">
        <button type="button" class="px-4 py-2 text-[13px] font-semibold border-b-2 transition-colors"
                :class="tab === 'charts' ? 'border-brand text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'"
                @click="tab = 'charts'">Charts</button>
        <button type="button" class="px-4 py-2 text-[13px] font-semibold border-b-2 transition-colors"
                :class="tab === 'records' ? 'border-brand text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'"
                @click="tab = 'records'">Risk Feed <span class="ml-1 font-mono opacity-60 text-[11px]" x-text="pagination.total ? `(${pagination.total})` : ''"></span></button>
    </div>
...
function rptOpsRisk() {
    return {
        tab: 'charts', // 'charts' | 'records' — declutters the 600-line page
        filters: { poe: '' },
        meta: {}, kpis: [], rows: [],
