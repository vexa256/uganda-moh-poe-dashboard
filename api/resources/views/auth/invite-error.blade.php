...
<head>
    ...
    <title>Invitation unavailable · PHEOC Command Centre</title>
...
<body class="min-h-screen grid place-items-center bg-muted/40 p-6">
    <div class="card max-w-md w-full">
        <div class="card-content !p-6 sm:!p-8 text-center space-y-3">
            ...
            <h1 class="text-xl font-bold">Invitation {{ $reason === 'expired' ? 'expired' : 'unavailable' }}</h1>
            <p class="text-[13px] text-muted-foreground">{{ $message }}</p>
            <div class="pt-3"><a href="{{ url('/login') }}" class="btn btn-outline">Go to sign in</a></div>
        </div>
    </div>
</body>
</html>
