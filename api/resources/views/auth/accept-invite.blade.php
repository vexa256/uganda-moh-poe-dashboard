{{-- /accept-invite?token=…&email=… · posts to /api/v2/auth/accept-invitation --}}
<!DOCTYPE html>
<html lang="en" class="h-full">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Accept invitation · {{ config('app.name', 'PHEOC Uganda') }}</title>
@include('admin.partials.theme')
<script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body class="min-h-full bg-background text-foreground antialiased" x-cloak x-data="acceptInvite()">
    ...
    <div class="w-full max-w-sm space-y-6">
        ...
        <div class="text-center space-y-2">
            ...
            <div>
                ...
                <p class="text-xs text-muted-foreground">Invitation</p>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-content p-6 text-center space-y-3">
                    ...
                    <div>
                        ...
                        <p class="description mt-1">Taking you to sign-in…</p>
                    </div>
                </div>
            </div>
        </template>
        ...
        {{-- Missing-token state --}}
        <template x-if="!done && (!token || token.length !== 64)">
            <div class="card">
                <div class="card-content p-6 space-y-3">
                    <div class="alert alert-destructive">
                        ...
                        <div>
                            <h4 class="alert-title">This link doesn't look right</h4>
                            <div class="alert-description">Open the invitation directly from your email, or ask the administrator who invited you to send a new one.</div>
                        </div>
                    </div>
        ...
        <template x-if="!done && token && token.length === 64">
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title text-base">Set your password</h2>
                    <p class="card-description">Pick a password that only you know. At least 12 characters.</p>
                </div>
                <div class="card-content space-y-4">
                    <div x-show="err" class="alert alert-destructive">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <div><h4 class="alert-title">We couldn't activate your account</h4><div class="alert-description" x-text="err"></div></div>
                    </div>
...
function acceptInvite(){
    return {
        ...
        async submit(){
            ...
            this.loading = true;
            try {
                const r = await fetch('{{ url('/api/v2/auth/accept-invitation') }}', {
                    method:'POST',
                ...
                this.err = b?.error || b?.errors?.password?.[0] || 'Your invitation may have expired. Ask for a new one.';
            } catch(e) {
